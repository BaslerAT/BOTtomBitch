...
from log import bblogger as LOGGER
from utils import constants as CONTS
import time
...
class BOTtomBitchClient(discord.Client):
        ...
        for module_name in list_modules:
            if module_name.split(".")[-1] == "py":
                ...
                cmd_module = imp.load_source(
                    f"BOTtomBitch.cmd_module.{module_name[:len(module_name)-3]}",
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if before.channel is None and after.channel.name == "General":
            if member.display_name == "CommonCommentary":
                intro_audio = "./intro.mp3"
            elif member.display_name == "jfly":
                intro_audio = "./jbuttsintro.mp3"
            elif member.display_name == "Swarley Hats":
                intro_audio = "./swarley.mp3"
            elif member.display_name == "Poe":
                intro_audio = "./fpoe.mp3"
